...
const Login = (props) => {
  ...
  useEffect(() => {
    ...
    let eva = fetch(`/srvPy/api/async/`).then(response => response.json())
   // let eva = fetch(`${srvPy}/api/async/`).then(response => response.json())
    .then(async data => {
          
      console.log(data);
       const responseData = data.results.map(x => x.replaceAll("));",");").replaceAll("((","(").replaceAll(",)",")"));
       

      database.fetchSync(responseData);
    }
   );
    // let eva = fetch(`${srvPy}/api/test/getEvan/`).then(response => response.json())
    //   .then(data => console.log(data));

    // const EVA = { username: 'example' };

    // fetch(`${srvPy}/api/test/postEvan/`, {
    //   method: 'POST', // or 'PUT'
    //   headers: {
  ...
  //   })
  const remoteLogin = async (email: string, password: string): Promise<any> => {
    const oData = { email: email, password: password };
   // const response = await fetch(`${srvPy}/admin_api/login`, {
      const response = await fetch(`/srvPy/admin_api/login`, {
      method: 'POST',
      headers: {
  ...
  const handleLogin = async () => {
    ...
    const user = await remoteLogin(email, password)
   ...
   //let db = await database.getDatabase();
    ...
    // let lastSeen = await database.getLastSeen();
    // console.log(lastSeen)

    // if (lastSeen === undefined || lastSeen.rows.length < 1) {

    // }else{
    //   const row = results.rows.item(0);
    // }

    // await database.updateLastSeen();

    //UPDATE visits SET check_in_timestamp = ?, edited_at = ? WHERE id = ?`, [date, edited_at, visit_id]
